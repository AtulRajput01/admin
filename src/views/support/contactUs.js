...
const Contact = () => {
  ...
  const fetchMessages = async (filter) => {
    try {
      const response = await axios.post(`http://www.taxidermyadmin.hunt30.com/api/getContact`,{read:filter});
  ...
  const handleToggleRead = async (id, status) => {
    try {
      await axios.put(`http://www.taxidermyadmin.hunt30.com/api/updateContact/${id}`, { read: status });
