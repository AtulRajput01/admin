...
const UserManagement = () => {
  ...
  const fetchUsers = async (searchUser) => {
    try {
      const response = await axios.post("www.taxidermyadmin.hunt30.com/api/vendor/getUser", { search: searchUser });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`www.taxidermyadmin.hunt30.com/api/vendor/deleteUser/${id}`);
