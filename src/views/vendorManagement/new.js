...
const OrderDetails = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await axios.get('http://www.taxidermyadmin.hunt30.com/api/OrderDetails/');
  ...
  const handleUpdateStatus = async () => {
    try {
      const response = await axios.put(`http://www.taxidermyadmin.hunt30.com/api/OrderDetails/${selectedOrder._id}`, { status: newStatus });
