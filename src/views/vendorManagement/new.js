...
const OrderDetails = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await axios.get('http://54.244.180.151:3002/api/OrderDetails/');
  ...
  const handleUpdateStatus = async () => {
    try {
      const response = await axios.put(`http://54.244.180.151:3002/api/OrderDetails/${selectedOrder._id}`, { status: newStatus });
