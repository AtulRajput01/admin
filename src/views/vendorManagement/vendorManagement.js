...
const UserManagement = () => {
  ...
  const fetchUsers = async (searchUser,newFilter) => {
    try {
      const response = await axios.post("https://www.taxiadmin.hunt30.com/api/vendor/getVendor", {
        search:searchUser,
        status: newFilter
      });
  ...
  const handleViewOrder = async (user) => {
    try {
      const response = await axios.get(`https://www.taxiadmin.hunt30.com/api/ShopDetails/shop/${user._id}`);
  ...
  const accepetVendor = async (id, status) => {
    const response = await axios.post('https://www.taxiadmin.hunt30.com/api/vendor/approveVendor', { id, status })
    fetchUsers();
  }
  ...
  const verifyShop = async (id, status) => {
    ...
    const res = await axios.post('https://www.taxiadmin.hunt30.com/api/ShopDetails/verifyShop', data)
    const response = await axios.get(`https://www.taxiadmin.hunt30.com/api/ShopDetails/shop/${selectUser}`);
    setSelectedVendor(response.data);
  }
