...
const UserManagement = () => {
  ...
  const fetchUsers = async (searchUser,newFilter) => {
    try {
      const response = await axios.post("http://www.taxidermyadmin.hunt30.com/api/vendor/getVendor", {
        search:searchUser,
        status: newFilter
      });
  ...
  const handleViewOrder = async (user) => {
    try {
      const response = await axios.get(`http://www.taxidermyadmin.hunt30.com/api/ShopDetails/shop/${user._id}`);
  ...
  const accepetVendor = async (id, status) => {
    const response = await axios.post('http://www.taxidermyadmin.hunt30.com/api/vendor/approveVendor', { id, status })
    fetchUsers();
  }
  ...
  const verifyShop = async (id, status) => {
    ...
    const res = await axios.post('http://www.taxidermyadmin.hunt30.com/api/ShopDetails/verifyShop', data)
    const response = await axios.get(`http://www.taxidermyadmin.hunt30.com/api/ShopDetails/shop/${selectUser}`);
    setSelectedVendor(response.data);
  }
                  ...
                  Array.isArray(selectedVendor) && selectedVendor.length > 0 ? (
                    selectedVendor.map((vendor, index) => (
                      <CTableRow key={vendor._id}>
                        ...
                        <CTableDataCell style={{ textAlign: "center" }}>
                          {vendor.shopLogo ? (
                            <img src={`http://www.taxidermyadmin.hunt30.com/${vendor.shopLogo}`} alt="Shop Logo" style={{ width: '50px', height: '50px' }} />
                          ) : "null"}
                        </CTableDataCell>
                        <CTableDataCell style={{ textAlign: "center" }}>
