...
const SubscriptionPlansPage = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        const response = await axios.get('http://www.taxidermyadmin.hunt30.com/api/subscription');
  ...
  const handleAddPlan = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`http://www.taxidermyadmin.hunt30.com/api/subscription/${formData._id}`, {
          ...formData,
          features: formData.features.split(',').map(feature => feature.trim())
        });
      } else {
        const response = await axios.post('http://www.taxidermyadmin.hunt30.com/api/subscription', {
          ...formData,
          features: formData.features.split(',').map(feature => feature.trim())
        });
  ...
  const handleDeletePlan = async (id) => {
    try {
      await axios.delete(`http://www.taxidermyadmin.hunt30.com/api/subscription/${id}`);
